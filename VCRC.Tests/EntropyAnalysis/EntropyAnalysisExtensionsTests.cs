...
public class EntropyAnalysisExtensionsTests
{
    ...
    {
        _indoor = Enumerable.Range(18, 5)
            .Select(i => i.DegreesCelsius()).ToList();
        _indoorExtended = Enumerable.Range(18, 6)
            .Select(i => i.DegreesCelsius()).ToList();
        _outdoor = Enumerable.Range(36, 5)
            .Select(i => i.DegreesCelsius()).ToList();
        _cycles = Enumerable.Range(0, _indoor.Count)
            .Select(i => CreateVCRC(_indoor[i], _outdoor[i])).ToList();
    }
    ...
    public void EntropyAnalysisInRange_Always_ReturnsAveragedEntropyAnalysisResult() =>
        _cycles.EntropyAnalysis(_indoor, _outdoor).Should().Be(
            _cycles.Select((c, i) => c.EntropyAnalysis(_indoor[i], _outdoor[i])).ToList().Average());
